...
source_segments = [
    ...
    # HYUNDAI.HYUNDAI_KIA_EV6 (+ QCOM GPS)
    ...
    ("TOYOTA3", "f7d7e3538cda1a2a|2021-08-16--08-55-34--6"),
    # HONDA.HONDA_CIVIC (NIDEC)
    ("HONDA", "eb140f119469d9ab|2021-06-12--10-46-24--27"),
    # HONDA.HONDA_ACCORD (BOSCH)
    ...
    ("SUBARU", "341dccd5359e3c97|2022-09-12--10-35-33--3"),
    ...
    ("VOLKSWAGEN", "de9592456ad7d144|2021-06-29--11-00-15--6"),
    # MAZDA.MAZDA_CX9_2021
    ("MAZDA", "bd6a637565e91581|2021-10-30--15-14-53--4"),
...
excluded_interfaces = ["mock", "tesla", "rivian", "byd"]
...
def run_test_process(data):
    ...
    if not args.upload_only:
        ...
        res, log_msgs = test_process(
            cfg, lr, segment, ref_log_path, cur_log_fn, args.ignore_fields, args.ignore_msgs)
        # save logs so we can upload when updating refs
        save_log(cur_log_fn, log_msgs)

    if args.update_refs or args.upload_only:
        print(f'Uploading: {os.path.basename(cur_log_fn)}')
        assert os.path.exists(cur_log_fn), f"Cannot find log to upload: {
            cur_log_fn}"
...
if __name__ == "__main__":
    ...
    all_procs = {
        cfg.proc_name for cfg in CONFIGS if cfg.proc_name not in EXCLUDED_PROCS}

    cpu_count = os.cpu_count() or 1

    parser = argparse.ArgumentParser(
        description="Regression test to identify changes in a process's output")
    ...
    full_test = (tested_procs == all_procs) and (tested_cars == all_cars) and all(
        len(x) == 0 for x in (args.ignore_fields, args.ignore_msgs))
    ...
    if full_test:
        untested = (set(interface_names) - set(excluded_interfaces)
                    ) - {c.lower() for c in tested_cars}
        assert len(untested) == 0, f"Cars missing routes: {str(untested)}"

    log_paths: defaultdict[str, dict[str, dict[str, str]]
                           ] = defaultdict(lambda: defaultdict(dict))
    with concurrent.futures.ProcessPoolExecutor(max_workers=args.jobs) as pool:
        if not args.upload_only:
            download_segments = [seg for car,
                                 seg in segments if car in tested_cars]
        ...
        for car_brand, segment in segments:
            ...
            for cfg in CONFIGS:
                ...
                cur_log_fn = os.path.join(FAKEDATA, f"{segment}_{
                                          cfg.proc_name}_{cur_commit}.zst")
                if args.update_refs:  # reference logs will not exist if routes were just regenerated
                    ref_log_path = get_url(
                        *segment.rsplit("--", 1,), "rlog.zst")
                else:
                    ref_log_fn = os.path.join(FAKEDATA, f"{segment}_{
                                              cfg.proc_name}_{ref_commit}.zst")
                    ref_log_path = ref_log_fn if os.path.exists(
                        ref_log_fn) else BASE_URL + os.path.basename(ref_log_fn)

                dat = None if args.upload_only else log_data[segment]
                pool_args.append(
                    (segment, cfg, args, cur_log_fn, ref_log_path, dat))
