...
void SpectraCamera::config_ife(int io_mem_handle, int fence, int request_id, int buf0_idx) {
  LOGD("config IFE request_id %d (init %d)", request_id, io_mem_handle == 0);
  /*
  ...
  */
  ...
  if (io_mem_handle != 0) {
    ...
    io_cfg[0].planes[0] = (struct cam_plane_cfg){
      ...
      .plane_stride = ALIGNED_SIZE(sensor->frame_width, 0x10),
      .slice_height = sensor->frame_height + sensor->extra_height,
    };
    io_cfg[0].planes[1] = (struct cam_plane_cfg){
      .width = sensor->frame_width,
      .height = sensor->frame_height/2 + sensor->extra_height,
      .plane_stride = ALIGNED_SIZE(sensor->frame_width, 0x10),
      .slice_height = ALIGNED_SIZE(sensor->frame_height/2 + sensor->extra_height, 0x10),
    };
    io_cfg[0].offsets[1] = (io_cfg[0].planes[0].plane_stride*io_cfg[0].planes[0].slice_height + io_cfg[0].planes[0].meta_size);
    io_cfg[0].format = CAM_FORMAT_NV21;  // TODO: why is this NV21 here and NV12 in the initial device acquire?
  ...
  // it'll swap IOVA addrs into our main cmd buffer
  {
    pkt->num_patches = 0;
    /*
    pkt->num_patches = request_id > 1 ? 0x2 : 0x8;
